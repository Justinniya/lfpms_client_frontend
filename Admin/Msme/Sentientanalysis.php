...
// Function to search comments based on user input (for the logged-in user only)
function searchComments($conn, $searchTerm)
{
  $sql = "SELECT f.name AS customer_name, f.comment, f.sentiment, 
          ...
          WHERE u.userid = :userid AND f.comment LIKE :searchTerm";

  $stmt = $conn->prepare($sql);
  $searchTerm = '%' . $searchTerm . '%'; // Prepare the search term for LIKE
  $stmt->bindParam(':userid', $_SESSION['id'], PDO::PARAM_INT); // Only for the logged-in user
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['search'])) {
  ...
  $searchResults = searchComments($conn, $searchTerm); // Perform search for logged-in user
}
