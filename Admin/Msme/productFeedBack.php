...
// Fetch products that belong to the logged-in MSME user
$productQuery = "SELECT product_id, productName FROM products WHERE msme_id = ?";
$stmt = $conn->prepare($productQuery);
$stmt->execute([$loggedInUserId]);
$products = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Ensure $products is not empty before accessing index 0
$selectedProductId = isset($_GET['product_id'])
  ? $_GET['product_id']
  : (!empty($products) ? $products[0]['product_id'] : null);

// Ensure category is defined
$category = isset($_GET['category']) ? $_GET['category'] : 'DefaultCategory';

function fetchFeedbackData($conn, $productId, $category, $loggedInUserId)
{
  if (!$productId) {
    return null; // Prevent errors if no product is found
  }
  ...
  $stmt->bindParam(':msme_id', $loggedInUserId, PDO::PARAM_INT);
...
// Fetch feedback data only if a valid product is selected
$feedbackData = $selectedProductId ? fetchFeedbackData($conn, $selectedProductId, $category, $loggedInUserId) : null;
              ...
              <div class="home-tab">
                <div class="d-sm-flex align-items-center justify-content-between border-bottom">
                  <ul class="nav nav-tabs" role="tablist">
                    <li class="nav-item">
                      <a style="margin-left:5px; margin-top: -35px;  " class="nav-link active ps-0" id="home-tab" data-bs-toggle="tab" href="#overview" role="tab" aria-controls="overview" aria-selected="true">Overview of Product Feedback</a>
                    </li>
                  </ul>
                </div>
                          ...
                          <div class="card-body">
                            ...
                            <?php if ($selectedProductId): ?>
                              ...
                              foreach ($categories as $category):
                                $data = fetchFeedbackData($conn, $selectedProductId, $category, $loggedInUser['userid']);
                              ?>
